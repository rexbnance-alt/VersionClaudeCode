...
function renderScriptItemView() {
  ...
  for (var c = 0; c < comments.length; c++) {
    var cm = comments[c];
    commentsHtml +=
      '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;padding:10px 12px;border-radius:8px;background:var(--surface);margin-bottom:6px">' +
        '<div style="flex:1;min-width:0">' +
          '<div style="font-size:11px;color:var(--muted);margin-bottom:3px">' + (cm.date ? (new Date(cm.date)).toLocaleString('en-US', {month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit'}) : '') + '</div>' +
          '<div style="font-size:13px;color:var(--text);line-height:1.5;white-space:pre-wrap">' + escapeHtml(cm.text) + '</div>' +
        '</div>' +
        '<button class="db-icon-btn" onclick="deleteScriptComment(' + c + ')" title="Delete" style="color:var(--muted);width:24px;height:24px;font-size:13px;flex-shrink:0">🗑</button>' +
      '</div>';
  }
    ...
    commentsHtml +=
      '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;padding:10px 12px;border-radius:8px;background:var(--surface);margin-bottom:6px">' +
        '<div style="flex:1;min-width:0">' +
          ...
          '<div style="font-size:13px;color:var(--text);line-height:1.5;white-space:pre-wrap">' + escapeHtml(cm.text) + '</div>' +
        '</div>' +
        '<button class="db-icon-btn" onclick="deleteComment(' + iIdx + ',' + c + ')" title="Delete" style="color:var(--muted);width:24px;height:24px;font-size:13px;flex-shrink:0">🗑</button>' +
      '</div>';
  }

...
import { getFirestore, doc, getDoc, collection, getDocs, addDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
const tfaApp = initializeApp(tfaFirebaseConfig, "tfa-app");
const tfaDb = getFirestore(tfaApp);
...
window.tfaLogin = async function(){
  ...
  try {
    const snap = await getDoc(doc(tfaDb,"whitelist",username));
...
async function tfaLoadAccounts(){
  ...
  try {
    const snap=await getDocs(collection(tfaDb,"users",tfaCurrentUser,"accounts"));
...
window.tfaAddAccount = async function(){
  ...
  try {
    const ref=await addDoc(collection(tfaDb,"users",tfaCurrentUser,"accounts"),{name,secret});
...
window.tfaDeleteAccount = async function(id){
  ...
  try {
    await deleteDoc(doc(tfaDb,"users",tfaCurrentUser,"accounts",id));
...
async function tfaHmacSHA1(k,d){const key=await crypto.subtle.importKey("raw",k,{name:"HMAC",hash:"SHA-1"},false,["sign"]);return new Uint8Array(await crypto.subtle.sign("HMAC",key,d));}
...
function tfaRenderAccounts(){
  ...
  if(!tfaAccounts.length){
    list.innerHTML="<div style=\"text-align:center;padding:32px 16px;color:var(--muted);font-size:13px\">🔑<br><br>No accounts yet.</div>";
    return;
  }
  list.innerHTML=tfaAccounts.map(a=>`
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:11px 14px;margin-bottom:7px;position:relative;cursor:pointer" id="tfa-card-${a.id}">
      <div style="font-size:11px;font-weight:700;color:var(--text);margin-bottom:4px;letter-spacing:.3px">${a.name.replace(/&/g,"&amp;").replace(/</g,"&lt;")}</div>
      <div style="display:flex;align-items:center;gap:10px">
        <div style="flex:1">
          <div id="tfa-code-${a.id}" onclick="tfaCopyCode('${a.id}')" style="font-family:monospace;font-size:15px;font-weight:500;letter-spacing:3px;color:var(--text);cursor:pointer" title="Click to copy">------</div>
      ...
      <button onclick="tfaDeleteAccount('${a.id}')" style="position:absolute;top:8px;right:8px;background:none;border:none;color:var(--muted);font-size:12px;cursor:pointer;padding:2px 6px;border-radius:6px" title="Remove">✕</button>
    </div>
  `).join("");
  tfaUpdateAllCodes();
}
...
// Auto-restore session when page loads
document.addEventListener("DOMContentLoaded", function(){
  if(tfaCurrentUser){
    const loginWrap=document.getElementById("tfa-login-wrap");
    const mainWrap=document.getElementById("tfa-main-wrap");
    if(loginWrap&&mainWrap) tfaShowMain();
  }
});
